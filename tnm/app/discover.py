...
ALLOW = {
    # --- 실측으로 살아남은 둘 ---
    "실적": ("실적", "매출액또는손익구조", "영업잠정실적", "영업실적", "손익구조"),
    "공급계약": ("단일판매공급계약", "공급계약체결", "수주"),
    ...
    "자본변동": ("유상증자결정", "무상증자결정", "자기주식취득", "자기주식처분",
                 "전환사채권발행결정", "신주인수권부사채권발행결정"),
}

# 명시적 제외 — allowlist 를 통과해 버리는 일상 공시들. '정정' 이 넓어서
# "분기보고서 정정" 같은 것이 들어오는데, 그건 매매 신호가 아니다.
DENY = ("분기보고서", "반기보고서", "사업보고서", "감사보고서", "결산",
...
def classify_report(report_nm: str) -> str | None:
    ...
    DENY 를 먼저 본다 — '분기보고서 정정' 처럼 두 목록에 다 걸리는 것은
    일상 공시 쪽이 맞다.
    """
