...
## 트레이딩 시스템 (`trading/` + `tnm/`)

반자동 한국주식 트레이딩 시스템(키움증권 REST/WS, **실계좌**). 이 저장소에서
가장 큰 하위 시스템으로, 별도 systemd 서비스로 돈다.

```
[발굴]  6개 소스(거래대금·등락률·급증·야간발굴·뉴스공시·수동)
          → 신호 원장 → 취합 점수 → 게이트 승격 → 감시목록  (단일 통로)
[매매]  감시목록(매매 tier) → 규칙 평가(orb·gap·momentum·pullback…)
          → 게이트 체인(잔고·일일가드·국면·롱전용·사이징) → 승인대기 → 사용자 승인 → 발주
[청산]  매매 데스크(2초, WS 실시간가) → 손절 자동 / 목표 승인제
[검증]  결정·관측 원장 축적 → 사전 확정 기준으로 사후 판정 → 유지/폐기
```

| 구성 | 포트 | 역할 |
|---|---|---|
| `trading.service` | :8600 | 매매 본체 — 발굴 엔진·신호·주문·원장·측정 도구 |
| `tnm.service` | :8602 | 뉴스·공시 수집·LLM 분류(발굴 엔진의 news 소스) |
| 대시보드 (:8701) | — | UI 페이지 `trading.js`(매매)·`scout.js`(발굴 엔진)·`backtest.js`(성과) |

설계의 뼈대는 **측정 거버넌스**다: 어떤 랭킹도 매수가능 무작위를 못 이긴다는
실측(191거래일) 위에서 점수 기반 승격을 폐기하고 게이트(유동성·체결가능성·
probation·상한)로만 매매 대상을 정하며, 새 기능은 관측 전용으로 들어와 사전에
못박은 기준으로 판정받은 뒤에만 실거래에 닿는다.

**상세 문서: [`docs/trading/`](docs/trading/README.md)** — 아키텍처 · 발굴 엔진 ·
매매 집행 · 측정 결과 원장 · 운영 런북 · 유보 결함 목록.
...
서비스/잡/감사/홈)·트레이딩·TNM·LLM·**JW 도구(jw-mcp)**·데일리 브리핑·날씨·Docker.

**조회 전용 원칙의 예외는 두 곳뿐이다** — LLM 모델 설치 승인
(`api_llm_models_decide`)과 jw-mcp 사용자 승인·비활성화(`api_jw_user_status`).
둘 다 대상 API 가 루프백 전용이고, 대시보드 세션 뒤에 있으며, 모든 변경이 감사
로그에 남는다. 새 제어 버튼을 추가하려면 같은 세 겹을 갖추고 근거를 코드에 적어야
한다.
