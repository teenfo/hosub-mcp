...
| `GET /v1/admin/models` | 설치된 모델 + 용량 + 쓰는 역할 + 최근 사용 + 삭제 차단 사유 |
...
## 모델을 직접 찾아 설치하고, 안 쓰면 지운다

대시보드 **LLM 모델** 페이지에서 한다(외부 요청을 기다릴 필요가 없다).

- **찾기** — `config/catalog.yaml` 의 큐레이션 목록. ollama.com 을 스크레이핑하지
  않는다(검색 API 가 없어 HTML 파싱에 기대야 하고, 그러면 남의 사이트 개편에
  게이트웨이가 끌려 죽는다). 목록에 없으면 **이름 직접 입력**
- **설치** — 새 pull 코드를 만들지 않았다. 기존 승인 파이프라인에 `approved` 로
  밀어 넣으면 모델 루프가 2초 안에 집어간다. 진행률·재시도·실패 처리 그대로
- **크기 게이트** — 추정 크기가 `MEM_BUDGET_GB` 를 넘으면 **설치 전에** 거부한다.
  안 그러면 잘 받아 놓고 실행할 때마다 잡이 죽는다
- **삭제** — 아래 다섯 중 하나라도 걸리면 409 로 막는다. `force` 는 없다

| 차단 사유 | 왜 |
|---|---|
| 그 모델을 쓰는 역할이 있다 | 지워도 다음 요청에서 곧바로 재설치 대기가 걸린다 — 역할을 먼저 바꿔야 한다 |
| 대기 중인 잡이 있다 | 지우면 그 잡들이 통째로 멈춘다 |
| 실행 중이다 | 진행 중인 추론이 깨진다 |
| 설치가 진행 중이다 | 부분 파일이 남는다 |
| 임베딩 역할이 쓴다 | 임베딩은 큐가 아니라 동기 경로라 소비자가 **즉시 503** 을 받는다 |

삭제하면 설치 요청 행 자체를 지운다. `ready` 로 두면 다음 탐지에서 되살아나
Slack 을 울리고, `rejected` 로 바꾸면 이후 잡이 "설치가 거부됨" 이라는 **거짓 사유**로
하드 실패한다. 이력은 감사 로그에 남는다.
