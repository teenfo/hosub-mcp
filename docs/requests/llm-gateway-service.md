...
`roles.yaml` 만 단일 소스로 두니 모델 교체에 PR → 머지 → 배포 → 재빌드가 필요했다.
"한 줄만 고치면 된다" 는 맞았지만 그 한 줄을 반영하는 데 배포 한 번이 들어서 실험이
사실상 불가능했다. 그래서 **YAML 을 기본값으로 두고 그 위에 SQLite 오버라이드를
얹는다**(대시보드에서 즉시 반영, 되돌리기 한 번이면 YAML 로 복귀).

지킨 경계:

- 덮어쓸 수 있는 것은 `model`·`lane`·`timeout`·`options`·`max_prompt_chars` 뿐이다.
  **`kind` 는 제외** — `generate`→`embed` 로 바꾸면 그 역할이 잡 큐와 메모리 예산을
  우회하는 동기 경로로 넘어간다(웹 버튼으로 21GB 모델을 그 경로에 올리는 셈).
  **`system` 도 제외** — 위의 "프롬프트는 호출자 소유" 원칙과 충돌한다
- **역할 삭제는 만들지 않는다.** `classify_news`(TNM)·`analyze_workout`(roxlogy)은
  레포 간 계약이고, 웹 버튼 하나로 남의 프로덕션이 404 를 받게 할 수는 없다.
  대시보드에서 만든 역할(`origin=db`)만 지울 수 있다
- **`services.yaml` 에는 오버라이드 층을 만들지 않는다.** 공개 도달 가능한 토큰에
  웹 UI 로 역할 권한을 주는 것은 권한 상승이다. 새 역할은 `hosub` 만 쓸 수 있고,
  외부 서비스에 열려면 여전히 PR 을 거친다
- **잡을 자기완결형으로 바꿨다.** 모델뿐 아니라 `options`·`timeout` 도 생성 시점에
  스냅샷한다. 안 그러면 큐에 있던 잡이 "옛 모델 + 새 옵션" 으로 돈다
- **드리프트가 보여야 한다.** `/v1/status.overrides.count` 가 0보다 크면 프로덕션이
  레포와 다르다는 뜻이고, 대시보드가 배지로 알리며 "roles.yaml 스니펫 복사" 를 준다.
  오버라이드는 `/data/llmgw.db` 에만 있으므로 **DB 를 백업본으로 되돌리면 모델
  선택도 조용히 되돌아간다** — 롤백 레버(`DELETE FROM role_overrides;`)와 함께
  `llm-gateway/README.md` 에 적어 뒀다

### 7-2. 관리 API 의 경계 (`/v1/admin/*`)

모델 삭제·설치·역할 교체가 생기면서 토큰 유출의 피해 범위가 "GPU 소모" 에서
"맥의 모델 전부 삭제 + 모든 역할을 엉뚱한 모델로 + 임의 수십 GB 다운로드" 로 커졌다.
토큰 게이트만으로는 부족하므로 **구조로 가둔다**:

1. 변경 계열은 전부 `/v1/admin/*` 으로 모은다
2. **Caddy 가 공개 경로(`/llm/*`)에서 이 접두사를 404 로 잘라낸다** — `route` 안에서
   `@api` 프록시보다 **앞**에 놓아야 한다(순서가 의미를 갖는다). 관리 기능은
   `127.0.0.1:8603` 으로만 닿고, 대시보드·MCP 가 쓰는 주소가 바로 그것이라 깨지는 게 없다
3. 앱 안에서도 `admin: true` 서비스만 통과시킨다 — 프록시만 믿지 않는다
4. 모든 변경이 대시보드 감사와 게이트웨이 `admin_audit` 양쪽에 남는다

> 이전 문구였던 "승인해도 설치되는 것은 PR 리뷰를 거친 역할이 참조하는 모델뿐" 은
> **자유 입력 설치가 생기면서 무효가 됐다.** 위 네 겹이 새 경계다.

### 7-3. 브라우저 탐색기의 경계 (철회 포함)

소비자가 계약을 기계 판독 형태로 가져갈 수 있게 `/v1/meta`·`/v1/openapi.json|yaml`·
`/v1/client/*` 를 열고, 파이썬이 아닌 소비자(roxlogy@Vercel)를 위해 브라우저
탐색기 `/v1/docs` 를 붙였다.

> **철회.** 이전 결정은 *"인증을 요구한다. 공개 경로(`/llm/v1/*`)에 놓이므로
> 스캐너에게 역할·모델·내부 구조를 알려줄 이유가 없고, 소비자는 어차피 토큰을
> 받는다"* 였다(`/v1/integration` 을 붙인 커밋). **`/v1/docs` 한 경로에서 이것을
> 뒤집는다** — 브라우저는 최상위 내비게이션에 `Authorization` 헤더를 싣지 못하므로,
> 껍데기를 인증 없이 열어 주지 않으면 토큰 입력 폼조차 보여줄 수 없다.

새 경계는 이렇다:

1. **공개되는 것은 정적 HTML 껍데기와 벤더 자산뿐이다.** 역할 이름·모델 이름·
   엔드포인트 목록이 그 파일에 없다 — 테스트가 단언한다
   (`llm-gateway/tests/test_docs_explorer.py`).
2. **계약과 실행은 여전히 토큰 뒤에 있다.** `/v1/openapi.*`·`/v1/meta`·
   `/v1/client/*` 는 전부 `_auth` 를 지난다. 화면에 뜨는 스펙은 입력한 토큰으로
   `fetch` 해 온 것이고, 그 토큰이 쓸 수 있는 역할만 들어 있다.
3. **같은 출처라 CORS 를 열지 않는다.** 15-1절의 "브라우저가 게이트웨이를 직접
   부르면 토큰이 노출된다" 는 기술적 이유가 그대로 유지된다 — 탐색기는 자기
   출처를 부른다.
4. **브라우저에 토큰을 넣지 말라는 규칙은 프로덕션 소비자 코드에 여전히 적용된다.**
   이건 개발용 탐색기이고, 페이지 자체에 그 구분을 배너로 적었다. 토큰은
   sessionStorage 에만 두고 서버에 저장하지 않는다.
5. **무인증 경로라 레이트리밋이 걸리지 않는다.** 그래서 정적 파일과 장기 캐시
   (`max-age=604800`)로만 구성한다. 컨테이너 로그는 이미 10MB×3 으로 회전한다.
6. **위험 범위는 그대로 "맥 GPU 소모"까지다**(15-1절). Try-it-out 은 실제 요청이라
   소비자의 분당 한도를 소모하지만, 그 한도가 상한이라는 사실은 변하지 않는다.

**계약을 손으로 적지 않는다.** `llm-gateway/README.md` 의 API 표와
`docs/integration.md` 의 엔드포인트 표가 둘 다 `/v1/meta` 계열을 빠뜨린 채
방치됐다 — 손 목록은 반드시 어긋난다. 그래서 메타데이터·스펙의 엔드포인트 재고는
`app.routes` 에서 유도하고, 라우트를 추가하고 요약을 달지 않으면 테스트가 실패한다.
권위 있는 목록은 문서가 아니라 `GET /v1/meta` 의 `endpoints` 다.

**관리 표면은 스키마 없이 목록만 낸다.** 소비자 `paths` 에 관리 경로를 넣으면
공개 경로에서 404 인 API 를 약속하는 셈이고, 절반만 적으면 계약이 두 곳에서
어긋난다. admin 토큰에는 `x-admin-endpoints` 로 `(method, path, summary)` 만
알린다 — 목록은 거짓말할 수 없다.

### 7-4. 소비자 토큰 관측 (철회 포함)

소비자 토큰은 `services.yaml` 에 **env 변수 이름만** 있고 값은 gitignore 된
`llm-gateway/.env` 에 있다. 그래서 "누가 등록돼 있고 그 토큰이 살아 있는가" 를 볼
수 있는 곳이 어디에도 없었다. 실제로 다음이 보이지 않았다:

- `.env` 값이 비면 그 서비스는 **경고 없이 죽는다**(`ServiceConfig.from_dict` 가
  빈 토큰으로 로드한다)
- 며칠째 호출이 없을 때 그게 토큰 문제인지 소비자 사정인지 구분할 수 없다
- `services.yaml` 에서 지운 이름의 사용 이력이 남아도 아무도 모른다

`GET /v1/admin/services` 가 이를 채운다 — 등록 현황·권한·한도·마지막 사용·
최근 호출, 그리고 **마스킹된 토큰과 sha256 앞 12자**.

> **철회.** 이전 원칙은 *"토큰은 표시하지 않는다 … 서비스별 토큰을 화면에 띄우면
> 조회 전용 대시보드가 비밀 노출 지점이 된다"* (#219) 였다. 운영자가 소비자에게
> 토큰을 다시 알려주려면 매번 서버에 들어가 `.env` 를 열어야 했고, 그 과정이
> 오히려 위험했다. **전체 값 열람을 허용하되 경계를 좁게 잡는다.**

새 경계:

1. **목록에는 전체 값이 절대 실리지 않는다.** 마스킹(`앞6…뒤6`)과 지문만.
   전 서비스 토큰이 한 응답에 담기는 상황을 만들지 않는다
2. **전체 값은 `GET /v1/admin/services/{name}/token` 으로 한 번에 하나만.**
   대시보드 쪽은 `POST` 로만 받는다 — 링크·프리페치·브라우저 기록으로 새지 않게
3. **두 곳에 기록한다.** 게이트웨이 `admin_audit`(`action=token_reveal`)과
   대시보드 감사(`risk=high`). **값은 어느 쪽에도 남기지 않는다** — 감사 로그가
   비밀 저장소가 되면 안 된다. 게이트웨이 로그에도 서비스 이름만 남는다
4. **관리 경로다.** `/v1/admin/*` 이므로 Caddy 가 공개 노출에서 404 로 자르고,
   앱 안에서도 `admin: true` 서비스(hosub)만 통과한다 — 기존 두 겹 그대로
5. **발급·회전·폐기는 여전히 없다.** 7-1절의 경계(*"공개 도달 가능한 토큰에 웹
   UI 로 역할 권한을 주는 것은 권한 상승"*)는 유지된다. 새 소비자는 여전히
   `services.yaml` PR → `.env` → 재기동이다. 이 절이 여는 것은 **관측과 열람뿐**
