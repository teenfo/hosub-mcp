...
# 둘 다 모르므로 **같은 값**이 정직하다.
#
# 0.667 인 이유(정정 2026-08-01 — 종전 주석 "소스 간 경쟁을 안 건드린다" 는
# 절반만 참이었다): 감쇠가 실제로 작동하는 지금(observed_at=배치 시각) 이 값은
# 다음 날 아침 0.667×0.64≈0.43 으로 승격선(0.35)을 유지하고 저녁(22시간)에
# 자연 소멸하는 수명을 만든다. 0.4 였다면 아침에 0.26 로 떨어져 야간 팔이
# 개장 전에 전멸한다. 부작용도 적는다: 무작위 팔이 종전 0.4→0.667 로 오르며
# 장중 소스 하위권과의 축출 경쟁 순서가 바뀌었다 — 이 비교는 측정된 적이
# 없고, 동점 축출의 무작위성은 promote 의 일자 시드 tie-break 가 지킨다.
#
