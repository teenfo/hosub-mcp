...
# 조건식 표본(`discovery.screen_pass`)의 강도. 무작위보다 위, 점수 표본
# (`score_strength(2.0)` = 0.667)보다 아래.
#
# **지금 이 값은 쓰이지 않는다** — `discovery.screen_fill` 기본값이 0 이라 조건식
# 팔이 돌지 않는다. 그 팔의 근거(백테스트에서 무작위를 이겼다)가 변동성 통제로
# 사라졌기 때문이다(정합 대조군 대비 +0.030R · t=+0.70). 자세한 것은
# `discovery.screen_pass` 의 docstring 에 남겼다.
#
# 값을 남겨 두는 이유는 팔을 되살릴 때 강도를 다시 고르지 않기 위해서다.
# **예측력 주장이 아니다** — 이 파일 맨 위 규약 그대로 자리 다툼의 순서일 뿐이다.
SCREEN_STRENGTH = 0.5
