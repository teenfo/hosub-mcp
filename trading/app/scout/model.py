...
# (`score_strength(2.0)` = 0.667)보다 아래에 둔다.
#
# **예측력 주장이 아니다** — 이 파일 맨 위 규약 그대로다. 자리 다툼에서 순서를
# 정할 뿐이고, 그 순서의 근거는 "사유가 몇 겹인가" 다. 조건식은 네 조건이 동시에
# 성립한 종목이라 무작위 추출보다 사유가 있고, 3규칙 상위처럼 우리가 만든 점수는
# 아니다. 4주 뒤 팔별 실현 R 이 나오면 이 상수가 아니라 **팔 자체**를 판정한다.
SCREEN_STRENGTH = 0.5
