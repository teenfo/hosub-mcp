...
#   ② 목표까지 50% 를 오면 **상승분의 30%**(lock_gain_pct 기본값) 를 확정한다
...
async def test_목표의_절반을_오면_상승분의_30퍼센트를_확정한다(env):
    """진입 10,000 · 현재 10,200(상승 200) → 10,000 + 200×0.3 = 10,060."""
    _open()
    spy = _Spy(ws={"005930": 10_200})           # 정확히 50%
    await desk.tick(spy.fresh, spy.rest, spy.execute, now=NOW)
    assert _lines()[0] == 10_060.0
...
async def test_지침_예시_그대로_계산된다(env, monkeypatch):
    ...
    monkeypatch.setitem(settings.CONFIG, "execution", {"desk": {
        "enabled": True, "trailing": True, "lock_gain_pct": 90,
        "max_gain_pct": 0}})                                       # 상한 끄고 본다
...
async def test_가격이_내려도_라인은_그_자리에_남는다(env):
    ...
    await desk.tick(spy.fresh, spy.rest, spy.execute, now=NOW)
    assert _lines()[0] == 10_075.0              # 10,000 + 250×0.3

    spy.ws["005930"] = 10_150                   # 되밀림
    stat = await desk.tick(spy.fresh, spy.rest, spy.execute, now=NOW)
    assert stat["lines"] == 0, "안 바뀌면 쓰지도 않는다"
    assert _lines()[0] == 10_075.0
...
async def test_따라_올라간_손절선에_닿으면_청산한다(env):
    ...
    _open()
    spy = _Spy(ws={"005930": 10_290})
    await desk.tick(spy.fresh, spy.rest, spy.execute, now=NOW)   # 손절선 10,090
    assert spy.exits == []

    spy.ws["005930"] = 10_080
    await desk.tick(spy.fresh, spy.rest, spy.execute, now=NOW)
    assert spy.exits == [("p1", "stop", 10_090.0)], "원래 손절 9,800 이 아니다"
...
async def test_숏도_절반을_오면_하락분의_30퍼센트를_확정한다(env):
    ledger.open_position({"id": "s2", "symbol": "034220", "side": "short",
                          "entry": 10_000, "stop": 10_200, "target": 9_600,
                          "rule": "orb", "qty": 5, "name": "034220"}, fill=10_000)
    spy = _Spy(ws={"034220": 9_800})            # 정확히 50%
    await desk.tick(spy.fresh, spy.rest, spy.execute, now=NOW)
    assert _lines("s2") == (9_940.0, 9_700.0)   # 10,000−200×0.3 · 상한 −3%
